...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static int size_impl(const Node *node) {
    if(node == nullptr)
        return 0;
    int leftSize = size_impl(node->left);
    int rightSize = size_impl(node->right);
  ...
  static int height_impl(const Node *node) {
    if(node == nullptr)
        return 0;
    int leftSize = height_impl(node->left);
    int rightSize = height_impl(node->right);
  ...
  static Node * find_impl(Node *node, const T &query, Compare less) {
      if(empty_impl(node))
          ...
          return find_impl(node->left, query, less);  //LESS THAN
      else if(less(node->datum, query))
          return find_impl(node->right, query, less); //GREATER THAN
