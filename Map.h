...
private:
  ...
  class PairComp {
    public:
       ...
       bool operator()(const Pair_type &pair_left, const Pair_type &pair_right) const {
            if(key_less(pair_left.first , pair_right.first))
                return true;
			return false;
        }
...
public:
  ...
  //       using "Value_type()".
    Iterator find(const Key_type& k) const {
        ...
        if (tree.find(dummy_pair) != tree.end())
			return tree.find(dummy_pair);
		return tree.end();
    }
  ...
  // HINT: http://www.cplusplus.com/reference/map/map/operator[]/
    Value_type& operator[](const Key_type& k) {
        Pair_type dummy_pair = {k, Value_type()};
        if(tree.find(dummy_pair) != tree.end())
            return tree.find(dummy_pair)->second;
        else {
			tree.insert(dummy_pair);
            return tree.find(dummy_pair)->second;
        }
    }
